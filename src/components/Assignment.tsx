      ...
      {tab === 'agent' ? (
        ...
        <div className="mt-4 grid gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="space-y-4">
            <section className="rounded-lg border border-line bg-surface p-4"><h3 className="text-[13px] font-semibold">The brief</h3><p className="mt-2 whitespace-pre-wrap text-[13px] leading-relaxed text-muted">{a.brief}</p><h4 className="mt-4 text-[12px] font-medium">Required deliverables</h4><ul className="mt-2 space-y-2 text-[12px] text-muted">{a.deliverables.map((d) => <li key={d} className="flex gap-2"><FileText size={12} className="mt-0.5 shrink-0" />{d}</li>)}</ul></section>
            <section><h3 className="mb-2 text-[13px] font-semibold">Source library <span className="font-normal text-muted">· {a.sources.length}</span></h3>{a.sources.map((s) => <details key={s.id} className="mb-2 rounded-lg border border-line bg-surface p-3"><summary className="cursor-pointer text-[12px] font-medium">{s.title}<span className="mt-1 block font-mono text-ai">{s.id}</span></summary><p className="mt-3 text-[12px] leading-relaxed text-muted">{s.excerpt}</p></details>)}{!a.sources.length && <p className="text-[12px] text-muted">Evidence is available in the linked operational workflow.</p>}</section>
            <section className="rounded-lg border border-dashed border-line p-3"><h3 className="text-[12px] font-semibold">What happens next</h3><p className="mt-2 text-[12px] leading-relaxed text-muted">{a.next}</p>{a.parentId && <button onClick={() => openAssignment(a.parentId!)} className="mt-2 text-[12px] text-ai hover:underline">View originating decision {a.parentId} ↗</button>}</section>
          </aside>
          <div className="min-w-0 space-y-4">
            {a.mode === 'type2' ? <>
              ...
              <section className="rounded-lg border border-line bg-surface p-4">
                ...
                {a.docId ? (
                  ...
                  <Btn className="mt-4" onClick={() => { openAssignment(null); setView('book'); }}>Open Book & rules <ArrowRight size={13} /></Btn>
                ) : a.status !== 'Done' && (
                  <>
            ...
            {!!a.revisions.length && <section><h3 className="mb-2 text-[13px] font-semibold">Submission & decision history</h3>{[...a.revisions].reverse().map((r) => <details key={r.version} className="mb-2 rounded border border-line bg-surface p-3"><summary className="cursor-pointer text-[12px]">v{r.version} · {r.by} · {r.decision ?? 'Awaiting review'}</summary>{r.reviewNote && <p className="mt-3 text-[13px] leading-relaxed">{r.reviewer} · {r.reviewedAt}<br />{r.reviewNote}</p>}<pre className="mt-3 whitespace-pre-wrap break-words text-[12px] text-muted">{JSON.stringify(r.content, null, 2)}</pre></details>)}</section>}
          </div>
        </div>
      )}
    </div>
