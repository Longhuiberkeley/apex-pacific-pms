...
export default function Funds(){
 const s=useStore();const [filter,setFilter]=useState<'Candidates'|'Invested'|'All'>('Candidates');
 return <div className="space-y-5"><header><h1 className="text-2xl font-semibold">Funds</h1><p className="mt-1 text-sm text-muted">Compare candidates, then open a fund to continue the work.</p></header><div className="flex gap-2">{(['Candidates','Invested','All'] as const).map(f=><button key={f} onClick={()=>setFilter(f)} className={`rounded border px-4 py-2 text-sm ${filter===f?'border-rail bg-rail text-white':'border-line bg-surface'}`}>{f}</button>)}</div>
 ...
 {filter!=='Invested'&&<Screening embedded/>}</div>;
}
