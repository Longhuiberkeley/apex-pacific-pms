...
import { Btn } from './Ui';
export default function Fees({fundId='HAL'}:{fundId?:string}) {
  const s = useStore();
  ...
  if (fundId!=='HAL') return <section className="rounded-lg border border-line bg-surface p-5"><h2 className="font-semibold">Management fees</h2><p className="mt-2 text-sm text-muted">No fee invoice and approved terms are on file for this fund yet.</p><Btn className="mt-3" onClick={()=>s.setFundTab('documents')}>View fund documents</Btn></section>;
  return <div className="space-y-5"><header><h1 className="text-2xl font-semibold">Management fee reconciliation</h1><p className="mt-1 text-sm text-muted">Reviewed source data → calculation → human decision</p></header>
    ...
    <section><h2 className="mb-3 font-semibold">Saved reconciliations</h2>{s.feeReviews.filter(r=>canRead(s.identity,r)).map(r=><details key={r.id} className="mb-2 rounded border border-line bg-surface p-4"><summary className="cursor-pointer text-sm">Reconciliation · {money(r.expected)} expected · reviewed by {r.reviewer}</summary><p className="mt-3 text-sm">{r.note}</p><details className="mt-3"><summary className="cursor-pointer text-xs text-muted">Technical details</summary><pre className="mt-3 overflow-x-auto text-xs">{JSON.stringify(r,null,2)}</pre></details></details>)}</section>
  </div>;
}
